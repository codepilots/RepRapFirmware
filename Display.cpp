...
bool Display::GCodeAvailable()
{
  return gcodeReadIndex != gcodeWriteIndex;
}

byte Display::ReadGCode()
{
  byte c;
...
void Display::LoadGcodeBuffer(const char* gc, bool convertWeb)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			if(gtp == ARRAY_SIZE(gcodeTempBuf) - 1)
			{
				...
				if (c != ' ' && !inComment)
				{
					platform->Message(BOTH_ERROR_MESSAGE, "Display: GCode local buffer overflow.\n");
					return;
				}
...
void Display::ProcessGcode(const char* gc)
{
  ...
  else
  {
	  ...
	  if (len > GetGcodeBufferSpace())
		  platform->Message(BOTH_ERROR_MESSAGE, "Display: GCode buffer overflow.\n");
	  else
	  {
...
void Display::GetJsonResponse(const char* request)
{
  jsonPointer = 0;
  writing = true;
...
void Display::Spin()
{
  ...
  if(writing)
  {
	  ...
	  {
		  platform->ClassReport("Display", longWait);
		  return;
	  }
  }
...
void Display::Exit()
{
  platform->Message(HOST_MESSAGE, "Display class exited.\n");
  active = false;
}

void Display::Diagnostics()
{
  platform->Message(HOST_MESSAGE, "Display Diagnostics:\n");
}


// Get the actual amount of gcode buffer space we have
unsigned int Display::GetGcodeBufferSpace() const
{
	return (gcodeReadIndex - gcodeWriteIndex - 1u) % gcodeBufLength;
}

// Get the amount of gcode buffer space we are going to report
unsigned int Display::GetReportedGcodeBufferSpace() const
{
...
void Display::WebDebug(bool wdb)
{
	webDebug = wdb;
}
